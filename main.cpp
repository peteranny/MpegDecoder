	...
	public:
		void read_macroblock(){
			...
			if(!macroblock_address_increment_huff){
				...
				macroblock_address_increment_huff->set_codeword(0, "00000001111");
				macroblock_address_increment_huff->set_codeword(255, "00000001000");
				macroblock_address_increment_huff->make_hash_table();
			}
			int macroblock_address_increment = macroblock_address_increment_huff->decode(nextbits(11));
			mpegFile->read_bits_as_num(macroblock_address_increment_huff->get_codelen(macroblock_address_increment));
			...
			switch(picture_coding_type){
				case 1: // intra-coded (I)
					...
					macroblock_type = macroblock_type_I_huff->decode(nextbits(2));
					mpegFile->read_bits_as_num(macroblock_type_I_huff->get_codelen(macroblock_type));
		...
		void read_block(int i){
			...
			if(pattern_code[i]){
				...
				if(picture_coding_type != 4){
					...
					int run, level;
					while(1){
						unsigned char dct_coeff_next = dct_coeff_huff->decode(nextbits(16));
						mpegFile->read_bits_as_num(dct_coeff_huff->get_codelen(dct_coeff_next));
