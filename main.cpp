...
class MpegDecoder{
	...
	public:
		void read_macroblock(){
			...
			for(int i = 0; i < macroblock_address_increment; i++){
				...
				// skipped macroblock
				if(i < macroblock_address_increment - 1){
					if(picture_coding_type == 2){ // P frame skipped macroblock
						...
						for(int m = 0; m < 16; m++) for(int n = 0; n < 16; n++){
							frame[y + m][x + n] = frame_past[y + m][x + n];
						}
					}
				}
				// process macroblock
...
#include "dct_dc_size_chrominance_huff.h"
		...
		void ref_using_motion_vec(int i){
